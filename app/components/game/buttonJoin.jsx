...
const ButtonJoin = ({ game, GetGame }) => {
  ...
  const handlerGame = async () => {
    try {
      const data = await axios(
        `http://localhost:3000/api/game/${Game._id}/user/${user._id}`
      );
  ...
  const changeOpen = async (open) => {
    try {
      const data = await axios(
        `http://localhost:3000/api/game/${Game._id}/user/${user._id}/isOpen/${open}`
      );
  ...
  const randomTeam = async () => {
    try {
      const data = await axios(
        `http://localhost:3000/api/game/${Game._id}/user/${user._id}/random`
      );
      console.log(data.data);
      const game1 = await axios.get(
        `http://localhost:3000/api/game/${Game._id}`
